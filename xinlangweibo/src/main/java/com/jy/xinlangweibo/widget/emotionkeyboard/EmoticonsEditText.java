package com.jy.xinlangweibo.widget.emotionkeyboard;
...
public class EmoticonsEditText extends EditText {
    ...
    public EmoticonsEditText(Context context, AttributeSet attrs) {
        ...
        if (emoticonBeanList == null) {
            ...
            dbHelper.cleanup();
            if (emoticonBeanList == null) {
            	Logger.showLog("没有匹配到emoticonBeanList:", "EmoticonsEditText");
                return;
            }
        }
    }
...
//arg0  改变以后的字符串 start的下标不是原文本的末尾，从start开始的count个字符替换长度为before的旧文本
    @Override
    protected void onTextChanged(CharSequence arg0, int start, int lengthBefore, int after) {
        ...
        if (after > 0) {
            int end = start + after;
            String keyStr = arg0.toString().substring(start, end);//提取的字符串 包含start 不包含 end
