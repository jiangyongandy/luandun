package com.jy.xinlangweibo.widget.pulltorefreshswipemenulistviewsample;
...
import com.jy.xinlangweibo.widget.swipemenulistview.SwipeMenuLayout;
import com.jy.xinlangweibo.widget.swipemenulistview.SwipeMenuView;
...
// 由当前按下的点得到当前点击的item
		...
		case MotionEvent.ACTION_MOVE:
			...
			if ((mTouchView == null || !mTouchView.isActive()) && Math.pow(dx, 2) / Math.pow(dy, 2) <= 3) {    // 满足条件判断为 y方向上的滑动
				if (getFirstVisiblePosition() == 0 && (mHeaderView.getVisiableHeight() > 0 || deltaY > 0)) {
					// the first item is showing, header has shown or pull down.  即判断为到顶下拉
					updateHeaderHeight(deltaY * OFFSET_RADIO);
					invokeOnScrolling();          //用户定义的 滚动监听
			...
			if (mTouchState == TOUCH_STATE_X) {    // x方向滑动
				if (mTouchView != null) {
					mTouchView.onSwipe(ev);
				}
				...
				super.onTouchEvent(ev);
				return true;
			} else if (mTouchState == TOUCH_STATE_NONE) {    // 还未确定方向
//				与自定义距离判断 是x方向滑动 还是y方向滑动
//				这里的判断不能这样写 因为这样判断导致触摸事件只有X和Y方向的滑动
// （X方向的滑动会取消onitemclick Y方向的滑动也会取消？）（间接加强了onitemclick的判断）
...
//				这样写使得除了X和Y方向的滑动 还有TOUCH_STATE_NONE状态 该状态为没有X和Y方向的滑动的默认状态
				if (Math.abs(dy) > MAX_Y) {
//				Logger.showLog("y方向滑动--------------","未确定方向  move");
					mTouchState = TOUCH_STATE_Y;
				} else if (dx > MAX_X) {
//				Logger.showLog("x方向滑动--------------","未确定方向 move");
					mTouchState = TOUCH_STATE_X;
					if (mOnSwipeListener != null) {
						mOnSwipeListener.onSwipeStart(mTouchPosition);
					}
		...
		case MotionEvent.ACTION_UP:
			...
			} else if (getFirstVisiblePosition() == 0) {   //到顶下拉释放
				// invoke refresh
