...
//这里是实现滑动的真正方法 （通过布局参数改变  dis 为左边隐藏部分的宽度）
...
//  startscroll（仅仅计算了坐标）开始执行后 真正的滑动在 computescroll中执行
    @Override
...
//       此处水平起始位置为0             距离为正数（此时的contentview的left坐标） 向右滚动 内容往右移
    ...
    public void smoothOpenMenu() {
        state = STATE_OPEN;
//      此处水平起始位置为正值（contentview 的left坐标) , 距离为正数menuview 的宽度  向左滚动  内容往左移  
//        mOpenScroller.startScroll(-mContentView.getLeft(), 0, mMenuView.getWidth(), 0, 350);
        mOpenScroller.startScroll(-mContentView.getLeft(), 0, mMenuView.getWidth()+mContentView.getLeft(), 0, 350);
        postInvalidate();
    }
...
//     重写 onmeasure         
//        主要为改变menuview    宽度mesaure mode为 UNSPECIFIED（父元素不对子元素施加任何束缚，子元素可以得到任意想要的大小）  size为0    高度为父布局高度
...
//    重写布局  布局了两个VIEW menuview在父布局右边
//    	这里没有调用父类的布局方法 （super.onLayout()）
